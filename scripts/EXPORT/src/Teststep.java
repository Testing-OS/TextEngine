...
public class Teststep  {
	...
	public void logStepExec(WebDriver  selenium, String xpath) throws IOException{
		TextEngine.addStep(scenario_id, this);
		...
		if(TextEngine.cloud.equals("BrowserStack")) {
			try {
				...
				if(status.equals("OK")) {
					if(object_Label.equals("Ecran"))
						retour.append(action_label).append(" (").append(screen_label);
					else
						retour.append(action_label).append(" (").append(object_Label);
					if(!param.equals(""))
						retour.append(", \\").append("\"").append(param).append("\\").append("\"");
					retour.append(")");
					}
				else
					retour.append(errorMessage);
	
				if(status.equals("Warning")) {
					jse.executeScript("browserstack_executor: {\"action\": \"annotate\", \"arguments\": {\"data\": \""+retour+"\", \"level\": \"debug\"}}");
					...
					jse.executeScript("browserstack_executor: {\"action\": \"annotate\", \"arguments\": {\"data\": \""+retour+"\", \"level\": \"error\"}}");
					jse.executeScript("browserstack_executor: {\"action\": \"setSessionStatus\", \"arguments\": {\"status\":\"failed\", \"reason\": \""+ errorMessage +"\"}}");
				}
				else
					jse.executeScript("browserstack_executor: {\"action\": \"annotate\", \"arguments\": {\"data\": \""+retour+"\", \"level\": \"debug\"}}");
			...
			try {
				...
				if(status.equals("Warning")) {
					jse.executeScript("sauce:context=" + retour);
		...
		if (Config.compteur_instance>2){
		 ...
		 if (status.equals("Warning")) {
			 ...
			 System.out.println ("Capture en warning : " + Config.dir_export + File.separator + testcase_label + File.separator + testcase_label + "_" + scenario_index + "_" + scenario_id + "_" + module_index + "_" + index_step + compteur_instance + "_WARNING.jpg");
			 BufferedImage screencapture = null;
				try {
			     ...
			     // Save as JPEG
		     File file = new File(Config.dir_export + File.separator + testcase_label + File.separator + testcase_label + "_" +  scenario_index  + "_" + scenario_id + "_" + module_index + "_" + index_step + compteur_instance + "_WARNING.jpg");
		     ImageIO.write(screencapture, "jpg", file);
		    
		 }
		 else if (status.equals("KO")) {
			 System.out.println ("Capture en KO : " + Config.dir_export + File.separator + testcase_label + File.separator + testcase_label  + "_" +  scenario_index  + "_" + scenario_id + "_" + module_index + "_" + index_step + compteur_instance + "_KO.jpg");
			 BufferedImage screencapture = null;
				try {
					screencapture = new Robot().createScreenCapture(
					 new Rectangle(Toolkit.getDefaultToolkit().getScreenSize()) );
			     ...
			     // Save as JPEG
		     File file = new File(Config.dir_export + File.separator + testcase_label + File.separator + testcase_label  + "_" + scenario_index + "_" + scenario_id + "_" + module_index + "_" + index_step + compteur_instance +  "_KO.jpg");
		     ImageIO.write(screencapture, "jpg", file);
