...
public class Scripts_techniques {
	...
	public static boolean WebPage_executer_bat(WebDriver selenium, Teststep teststep) throws IOException {
		...
		try {
			if (Pattern.matches(".+\\.bat", parameter)) {
				try {
	...
	public static boolean  WebPage_getsms(WebDriver selenium, Teststep teststep) throws IOException {
		...
		try {
			...
			try {
				...
				caps.setCapability(MobileCapabilityType.DEVICE_NAME, Config.deviceId);
				// Replace with your device name or ID emulator-5554 RZCT50FRL2F
				// Initialize Appium Driver
				driver = new AndroidDriver(new URL(Config.appium_url), caps);
	...
	public static boolean  WebPage_PressKey(WebDriver selenium, Teststep teststep) throws IOException {
	...
	public static boolean WebObject_selectbytext(final WebDriver driver, Teststep teststep)
	{
		...
		try {
			...
			boolean isNumeric = isANumberBetweenCrochet(teststep.param);
			Boolean datapoolparam = (teststep.param.charAt(0) == '[' && teststep.param.charAt(teststep.param.length()-1) == ']') && !isNumeric;
			String tryProperty = "";
			if (datapool && isJVExecution(teststep.testcase_label)) {
				tryProperty = Fonctions.getParameter(file_jv, Config.compteur_instance, Config.compteur_params);
			} else {
	...
	public static boolean isANumberBetweenCrochet(String input) {
		if (input == null || !input.contains("[") || !input.contains("]")) {
			return false;
		}

		String valueInBrackets = input.substring(1, input.length()-1);
		try {
			Integer.parseInt(valueInBrackets); // Supporte les entiers et les décimaux
			return true;
		} catch (NumberFormatException e) {
			return false;
		}
    ...
    public static boolean WebPage_comparescreenshot(WebDriver selenium, Teststep teststep) throws IOException {
        ...
        path = Config.additional_files + "/screenshots";
        file = new File(path);
