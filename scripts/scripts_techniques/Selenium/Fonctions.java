...
if (tag.getAttribute("frameid")!= null)
	...
	public static Hashtable<String, String> getObjectProperties (String htmlfile_name) {
		...
		try {
			...
			BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(new FileInputStream(jsonFile), "UTF8"));
			Gson gson = new Gson();
			...
			while (it.hasNext()) {
				...
				if (htmlfile_name.equals(object.getTechLabel())) {
					...
					for (int i=0; i < props.size(); i++) {
						...
						System.out.println (prop.getPropValue());
	...
	public static void setObjectProperties(String techlabel, String newPropName, String newPropValue) {
		...
		try {
			...
			//Recherche d'un objet par son techlabel
			for (ktObject object : objects) {
				if (techlabel.equals(object.getTechLabel())) {
					System.out.println("Object trouvé dans le fichier JSON : " + object.getLabel());
					//Recuperation de toute les propriété de l'objet
					List<ktProp> props = object.getListOfProps();
					boolean propertyUpdate = false;

					for (ktProp prop : props) {
						if (newPropName.equals(prop.getPropName())) {
							// Si la propriété existe deja on met a jour sa valeur
					...
					if (!propertyUpdate) {
						...
						props.add(new ktProp(newPropName, newPropValue));
					}
					ktObjects objs = new ktObjects();
					objs.setListOfObjects(objects);


					// Écrire les données mises à jour dans le fichier JSON
					try (Writer writer = new FileWriter(jsonFile)) {
						Gson gsonPretty = new GsonBuilder().setPrettyPrinting().create();
						gsonPretty.toJson(objs, writer);
					}

					System.out.println("Nouvelle propriété ajoutée avec succès à l'objet : " + object.getLabel());
				}
			}
		} catch (IOException e) {
			e.printStackTrace();
...
// public static void switchToWebViewContext(WebDriver driver) {
...
// }
	public static void highLighterMethod(WebDriver driver, WebElement element) throws InterruptedException{
		...
		String bg_border_color = element.getCssValue("border-top-color");
		String colorChanged = "rgb(232, 131, 99)";
		for(int i=0; i<3; i++)
		{
			js.executeScript("arguments[0].setAttribute('style', 'background-color: "+colorChanged+"; border: 3px solid "+colorChanged+";');", element);
			Thread.sleep(250);
			js.executeScript("arguments[0].setAttribute('style', 'background-color: "+bgcolor+"; border: 0px solid "+bg_border_color+";');", element);
			try {
				Thread.sleep(250);
			} catch(InterruptedException e) {
			}
	...
	public static boolean AddNewObject(String PropName, String PropValue) throws FileNotFoundException {
		boolean status = true;

		BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(new FileInputStream(Config.file_objects)));
		Gson gson = new Gson();
		ktObjects json = gson.fromJson(bufferedReader, ktObjects.class);

		List<ktObject> objects = json.getListOfObjects();
		List<ktProp> Props = new ArrayList<>();
		Props.add(new ktProp(PropName, PropValue));
		String label = PropName + "_" + PropValue;
		String techlabel = label + ".htm";

		ktObject NewObj = new ktObject(label,techlabel, Props);
		objects.add(NewObj);
		ktObjects objs = new ktObjects();
		objs.setListOfObjects(objects);

		try (Writer writer = new FileWriter(Config.file_objects)) {
			Gson gsonPretty = new GsonBuilder().setPrettyPrinting().create();
			gsonPretty.toJson(objs, writer);
		}
		catch (Exception e){
	...
	public static List<String> extractTechLabels(File jsonFile) {
		...
		try {
			...
			if (objectsNode.isArray()) {
				...
				while (iterator.hasNext()) {
					...
					// Accéder à la propriété "techlabel" de chaque objet
					JsonNode techLabelNode = objectNode.get("techlabel");

					if (techLabelNode != null) {
						techLabels.add(techLabelNode.asText());
					}
				}
			}
		} catch (Exception e) {
	...
	//Cette méthode permet après chaque action si un Xpath est récupéré de l'ajouter a la liste des Xpath sauvegardé
	public static void addxpathtoCSV(String csvFile, String name,String xpath) {
		//Creation des 2 lignes composant le CSV (1 pour le nom des actions et 1 pour les xpath)
		StringBuilder Sname = new StringBuilder();
		StringBuilder Sxpath = new StringBuilder();

		try (BufferedReader reader = new BufferedReader(new FileReader(csvFile))) {
			String line;
			//Lecture des lignes du fichier, ajouts de la premiere valeurs
			line = reader.readLine();
			System.out.println("ligne : "+ line);
			//Si le fichier est vide
			if (Objects.equals(line, null)){
				Sname.append(name).append("\n");
				line = reader.readLine();
				Sxpath.append(xpath).append("\n");


			}
			//Ajouts des valeurs à la suite des précédentes si le fichier n'est pas vide
			else {
				Sname.append(line).append(";").append(name).append("\n");
				line = reader.readLine();
				Sxpath.append(line).append(";").append(xpath).append("\n");

			}
			//Concaténation des noms et des xpaths
			Sname.append(Sxpath);
	...
	public static void SaveXpath(String parcours_name, String techlabel, String CSVfile){
		String outfile = Config.dir_export + File.separator+ parcours_name + File.separator+ parcours_name+".OUT";
		String xpath = "";

		try (BufferedReader lecteur = new BufferedReader(new FileReader(outfile))) {
			String Ligne = "";
			for (int i = 0; i < Config.compteur_params; i++) {
				Ligne = lecteur.readLine();
			}

			if (Ligne != null) {
				String[] morceaux = Ligne.split(Config.outseparator);

				if (morceaux.length >= 28) {
					xpath = morceaux[27];
					System.out.println("Valeur du xpath : " + xpath);
					setObjectProperties(techlabel, "XpathSave", xpath);
				} else {
					System.out.println("Pas de Xpath.");
				}
			} else {
		...
		//if(!Objects.equals(xpath, "")){
			Fonctions.addxpathtoCSV(CSVfile, techlabel, xpath);
		//}
	}
}
